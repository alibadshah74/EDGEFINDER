...
export function DashboardLayout() {
  ...
  const { collapsed, search, assetFilter, category, biasFilter } = useDashboardStore()
    ...
    <div className="min-h-screen bg-[#080b10] text-slate-100">
      ...
      <div className="relative transition-[padding] duration-300" style={{ paddingLeft: collapsed ? 54 : 196 }}>
        ...
        <main className="p-2.5 lg:p-3">
          {loading ? <LoadingSkeleton /> : <MarketHeatmapTable data={filteredData} compact={collapsed} />}
        </main>
      </div>
    </div>
  )
