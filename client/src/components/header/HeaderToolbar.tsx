...
export function HeaderToolbar() {
  const { search, setSearch, assetFilter, setAssetFilter, category, setCategory, biasFilter, setBiasFilter, activeNav, scoreAsset, setScoreAsset, dateRange, setDateRange } = useDashboardStore()
  ...
  return (
    <header className="sticky top-0 z-20 flex min-h-14 items-center gap-2 border-b border-white/10 bg-[#0b111a]/86 px-3 backdrop-blur-xl lg:px-4">
      <div className="flex min-w-0 items-center gap-2">
        <div className="grid h-8 w-8 shrink-0 place-items-center rounded border border-emerald-200/20 bg-gradient-to-br from-white/10 to-emerald-300/10">
        ...
        <div className="hidden min-w-0 xl:block">
          ...
          <p className="truncate text-[10px] text-slate-500">{isScorePage ? selectedAsset?.name ?? 'Score analytics workspace' : 'Synthetic macro signal workspace'}</p>
        </div>
      </div>
